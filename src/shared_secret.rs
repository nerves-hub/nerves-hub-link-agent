...
/// with an unset clock cannot authenticate at all. On a device with no RTC the clock reads
...
mod tests {
    ...
    // Produced by an implementation a running NervesHub accepted,
    ...
    fn salt_matches_the_server() {
        assert_eq!(
            hex(salt("SHA256-1000-32", KEY, SIGNED_AT).as_bytes()),
            SALT_HEX
        );
    }
    ...
    fn payload_matches_term_to_binary() {
        assert_eq!(
            hex(&payload(IDENTIFIER, SIGNED_AT * 1000, 86_400)),
            PAYLOAD_HEX
        );
    }
    ...
    fn token_matches() {
        ...
        pbkdf2::pbkdf2_hmac::<Sha256>(SECRET.as_bytes(), salt.as_bytes(), 1000, &mut derived);
        assert_eq!(
            token(&derived, &payload(IDENTIFIER, SIGNED_AT * 1000, 86_400)),
            TOKEN
        );
    }
