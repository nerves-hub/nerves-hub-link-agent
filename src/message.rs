...
pub mod event {
    ...
    /// Device -> server: the running firmware has proven itself. Paired with
    /// whatever the update tool needs to release the bootloader's rollback.
    ...
    /// Both directions: a support script to run, and its result. Requires the
    /// device to report `device_api_version >= 2.1.0`; below that NervesHub
